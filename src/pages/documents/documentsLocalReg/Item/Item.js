...
class Item extends Component {
    ...
    componentDidMount() {
        ...
        return fetch(`${config.api.apiRest}/requests/getLocalRegulationsDocument/${id}`, fetchConf)
    ...
    sendSave() {
        ...
        return fetch(`${config.api.apiRest}/requests/updateLocalRegulationsDocument/${this.props.match.params.id}`, fetchConf)
            .then((response) => {
                if (response.ok)
    ...
    deleteDocument() {
        ...
        return fetch(`${config.api.apiRest}/requests/deleteLocalRegulationsDocument/${this.props.match.params.id}`, fetchConf)
            .then((response) => {
                if (response.ok)
                    return history.push('/app/documents/local-reg');
        ...
        return (
            <div>
                ...
                <Card body outline color="primary" className="mt-2">
                    <ItemStatuses itemId={this.state.document._id} save={this.sendSave} closeLink="documents/local-reg" tgModal={this.toggleModal} />
